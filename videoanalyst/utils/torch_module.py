...
def move_data_to_device(data_dict: Dict, dev: torch.device, single_gpu=False):
    for k in data_dict:
        # print ('type(k).__name__ = ', type(k).__name__)
        # print ('isinstance(k, dict) = ', isinstance(k, dict))
        if isinstance(data_dict[k], dict):
            for mm in data_dict[k]:
                if single_gpu:
                    data_dict[k][mm] = data_dict[k][mm].cuda() # to(dev)
                else:
                    data_dict[k][mm] = data_dict[k][mm].to(dev)
                    
        else:
            if single_gpu:
                data_dict[k] = data_dict[k].cuda() # to(dev)
            else:
