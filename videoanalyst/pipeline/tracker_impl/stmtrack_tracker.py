...
class STMTrackTracker(PipelineBase):
    ...
    def getTemplateFea(self, im_fc, im_hsi, im_material, target_pos, target_sz):
        m_size = self._hp_m_size

        scale_m = math.sqrt(np.prod(target_sz) / np.prod(self._state['base_target_sz']))
        im_fc_t_crop, _ = get_crop_single(im_fc, target_pos, scale_m, m_size, self._state['avg_chans_fc'])
        im_hsi_t_crop, _ = get_crop_single(im_hsi, target_pos, scale_m, m_size, self._state['avg_chans_hsi'])
        im_material_t_crop, real_scale = get_crop_single(im_material, target_pos, scale_m, m_size, self._state['avg_chans_material'])

        with torch.no_grad():
            data_fc = imarray_to_tensor(im_fc_t_crop).to(self.device)
            data_hsi = imarray_to_tensor(im_hsi_t_crop).to(self.device)
            data_material = imarray_to_tensor(im_material_t_crop).to(self.device)
            
            bbox_m = np.concatenate([np.array([(m_size - 1) / 2, (m_size - 1) / 2]),
                                    target_sz * real_scale], axis=0)
            fg_bg_label_map = self.create_fg_bg_label_map(bbox_m, m_size)
            ft_fc, ft_mask, ft_pos, ft_hsi, ft_material = self._model(data_fc, data_hsi, data_material, fg_bg_label_map, phase='template')
    ...
    def init(self, fc_image, hsi_image, material_image, state):
        ...
        self._state['im_h'] = fc_image.shape[0]
        self._state['im_w'] = fc_image.shape[1]
        ...
        self._state['avg_chans_fc'] = (np.mean(fc_image[..., 0]), np.mean(fc_image[..., 1]), np.mean(fc_image[..., 2]))
        self._state['avg_chans_hsi'] = (np.mean(hsi_image[..., 0]), np.mean(hsi_image[..., 1]), np.mean(fc_image[..., 2]))
        self._state['avg_chans_material'] = (np.mean(material_image[..., 0]), np.mean(material_image[..., 1]), np.mean(fc_image[..., 2]))
        ...
        ft_fc, ft_mask, ft_pos, ft_hsi, ft_material = self.getTemplateFea(fc_image, hsi_image, material_image, target_pos, target_sz)
        self._state['template_fc'] = ft_fc
        self._state['template_mask'] = ft_mask
        self._state['template_pos'] = ft_pos
        self._state['template_hsi'] = ft_hsi
        self._state['template_material'] = ft_material
    ...
    def track(self,
              fc_image, hsi_image, material_image,
              target_pos,
              target_sz,
              features,
              update_state=False,
        ...
        if 'avg_chans' in kwargs:
            ...
            avg_chans_fc = self._state['avg_chans_fc']
            avg_chans_hsi = self._state['avg_chans_hsi']
            avg_chans_material = self._state['avg_chans_material']

        q_size = self._hp_q_size

        phase_track = self._hyper_params['phase_track']
        im_fc_s_crop, _ = get_crop_single(fc_image, target_pos, self._state['target_scale'], q_size, avg_chans_fc)
        im_hsi_s_crop, _ = get_crop_single(hsi_image, target_pos, self._state['target_scale'], q_size, avg_chans_hsi)
        im_material_s_crop, scale_q = get_crop_single(material_image, target_pos, self._state['target_scale'], q_size, avg_chans_material)
        self._state["scale_q"] = deepcopy(scale_q)

        with torch.no_grad():
            score, box, cls, ctr, extra = self._model(
                imarray_to_tensor(im_fc_s_crop).to(self.device),
                imarray_to_tensor(im_hsi_s_crop).to(self.device),
                imarray_to_tensor(im_material_s_crop).to(self.device),
                self._state['template_fc'],
                self._state['template_mask'],
                self._state['template_pos'],
                self._state['template_hsi'],
                self._state['template_material'],
                phase=phase_track)
        ...
        if False:
            bbox0 = box_wh[:, 0].reshape(25,25)
            bbox1 = box_wh[:, 1].reshape(25,25)
            bbox2 = box_wh[:, 2].reshape(25,25)
            bbox3 = box_wh[:, 3].reshape(25,25)
            cv2.imwrite('score.jpg', getZhiXinPic(score.reshape(25,25)))
            cv2.imwrite('bbox0.jpg', getZhiXinPic(normalize(bbox0)))
            cv2.imwrite('bbox1.jpg', getZhiXinPic(normalize(bbox1)))
            cv2.imwrite('bbox2.jpg', getZhiXinPic(normalize(bbox2)))
            cv2.imwrite('bbox3.jpg', getZhiXinPic(normalize(bbox3)))
        # raise Exception
        # score post-processing
        best_pscore_id, pscore, penalty = self._postprocess_score(
            score, box_wh, target_sz, scale_q, im_hsi_s_crop)
        # box post-processing
    ...
    def update(self, fc_image, hsi_image, material_image, state=None):
        ...
        # target_pos, target_sz = self.track(fc_image, hsi_image, material_image,
        #                                    target_pos_prior,
        #                                    target_sz_prior,
        #                                    features,
        #                                    update_state=True)
