...
class GOT10k(object):
    ...
    def __init__(self,
        ...
        self.hsiDataRootDir = '/nas_data/lizf/HOT/dataset/train/'

        self.seq_names = os.listdir(self.hsiDataRootDir)
        print ('self.seq_names = ', self.seq_names)

    def get_img_files(self, fold_path):
        filenameListTmp = os.listdir(fold_path)
        filenameListTmp.sort()
        filenameList = []
        for filename in filenameListTmp:
            if filename.find('.txt') != -1: continue
            else: filenameList.append(filename)
        return filenameList
    def get_gt_txt(self, gt_name):
        f = open(gt_name,'r')
        gt_arr = []
        gt_res = f.readlines()
        for gt in gt_res:
    ...
    def __getitem__(self, index):
        ...
        hsi_img_files = self.get_img_files(os.path.join(self.hsiDataRootDir, seq_name, 'HSI'))
        hsi_img_files = [os.path.join(self.hsiDataRootDir, seq_name, 'HSI', img_file ) for img_file in hsi_img_files]

        fc_img_files = self.get_img_files(os.path.join(self.hsiDataRootDir, seq_name, 'HSI-FalseColor'))
        fc_img_files = [os.path.join(self.hsiDataRootDir, seq_name, 'HSI-FalseColor', img_file ) for img_file in fc_img_files]
        
        material_img_files_tmp = self.get_img_files(os.path.join(self.hsiDataRootDir, seq_name, 'Material-R6'))
        material_img_files = []
        for dd in material_img_files_tmp:
            if dd.find('.png') != -1:
                material_img_files.append(dd)
        # print ('material_img_files = ', material_img_files)

        material_img_files = [os.path.join(self.hsiDataRootDir, seq_name, 'Material-R6', img_file ) for img_file in material_img_files]
        # material_img_files = self.get_img_files(os.path.join(self.hsiDataRootDir, seq_name, 'Material'))

        anno = self.get_gt_txt(os.path.join(self.hsiDataRootDir, seq_name, 'HSI', 'groundtruth_rect.txt'))
        if self.subset == 'test' and (anno.size // 4 == 1):
            anno = anno.reshape(-1, 4)
            # anno = anno[np.newaxis, :]
        else:
            if len(material_img_files) != len(anno):
                print ('seq_name = ', seq_name)
                pass
            assert len(material_img_files) == len(anno)
            assert len(fc_img_files) == len(anno)
            assert len(hsi_img_files) == len(anno)

        if self.return_meta:
            meta = GOT10k.data_dict[self.subset][seq_name]["meta"]
            return img_files, anno, meta
        else:
            return hsi_img_files, material_img_files, fc_img_files, anno
    ...
    def _get_cache_path(self, cache_path: str = None):
        ...
        if (cache_path is None) or (not os.path.isfile(cache_path)):
            ...
            if self.root_dir.find('/data/compute03') >= 0 and not os.path.exists('/mnt/ssddata'):
                ...
                pass
            cache_path = os.path.join(self.root_dir, self.subset + "{}.pkl".format(nfs_suffix))
        return cache_path
