...
class TrackPairSampler(SamplerBase):
    ...
    def __getitem__(self, item) -> dict:
        ...
        while any(list(map(self.data_filter, data_ms))) or self.data_filter(data2):
            if is_negative_pair:
                data_m, data2, is_static_image_m = self._sample_neg_track_frames()
            else:
                data_m, data2, is_static_image_m = self._sample_track_pair()
            data_m["hsi_img"] = load_image(data_m["hsi_img"])
            data_m["material_img"] = load_image(data_m["material_img"])
            data_m["fc_img"] = load_image(data_m["fc_img"])
            data2["hsi_img"] = load_image(data2["hsi_img"])
            data2["material_img"] = load_image(data2["material_img"])
            data2["fc_img"] = load_image(data2["fc_img"])
            sample_try_num += 1
        sampled_data = dict(
            data1=data_m,
            data2=data2,
