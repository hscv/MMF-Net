...
def build(task: str, cfg: CfgNode, optimizer, dataloader, devs=[],
    ...
    name = cfg.name
    if task == "vos":
        trainer = MODULES[name](optimizer, dataloader, devs, monitors, tracker)
    else:
        trainer = MODULES[name](optimizer, dataloader, devs, monitors)
    hps = trainer.get_hps()
